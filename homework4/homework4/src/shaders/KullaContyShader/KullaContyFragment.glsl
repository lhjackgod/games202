...
float DistributionGGX(vec3 N, vec3 H, float roughness)
{
    ...
    float right = (a * a - 1.0) ;
    float denom = noh * noh * right + 1.0;
    denom = denom * denom * PI;
    return a * a / max(0.001, denom);
...
{
    float VoN = max(0.0, dot(N, V));
    float LoN = max(0.0, dot(N, L));
    // TODO: To calculate Smith G here

    return GeometrySchlickGGX(VoN, roughness) * GeometrySchlickGGX(LoN, roughness);
}

vec3 fresnelSchlick(vec3 F0, vec3 V, vec3 H)
{
    vec3 F = F0 + (vec3(1.0) - F0) * pow(clamp((1.0 - max(0.0, dot(V, H))), 0.0, 1.0), 5.0);
