...
bool outScreen(vec3 Pos){
  ...
  return any(bvec4(lessThan(uv, vec2(0.0, 0.0)), greaterThan(uv, vec2(1.0, 1.0))));
}
bool Front(vec3 wpos){
  vec2 uv = GetScreenCoordinate(wpos);
  float wpos_depth = GetDepth(wpos);
  float uv_depth = GetGBufferDepth(uv);
  return wpos_depth < uv_depth;
}
